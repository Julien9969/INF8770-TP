...
input_batch = input_tensor.unsqueeze(0).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))  # Ajout d'une dimension de batch : 1 x 3 x 224 x 224                                                   # mode évaluation pour désactiver certaines couches comme BatchNorm ou Dropout
...
output = torch.nn.functional.normalize(rearrange(output, 'b d h w -> b (d h w)'))  # 512

# Visualisation
plt.plot(output.cpu().numpy().flatten())
